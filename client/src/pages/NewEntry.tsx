...
import { ArrowLeft, HardHat, Check, AlertTriangle, ChevronRight, Save } from "lucide-react";
...
export default function NewEntry() {
  ...
  const handleSubmit = () => {
    ...
    // ARCHITECTURAL NOTE: Zoho CRM API call would be injected here.
    // Example: await zohoApi.createRecord('Time_Entries', submissionData);
    console.log("Submitting to Office:", submissionData);

    toast({
      title: "Entry Submitted",
      description: "Your time entry has been sent to the office.",
      duration: 3000,
    });
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-200 flex justify-center">
        <div className="w-full max-w-md flex gap-3">
          {step < 3 ? (
             ...
             >
               Submit to Office <Save className="ml-2" size={20} />
             </PrimaryButton>
          )}
        </div>
      </div>
